...
//displaying filter buttons dynamically
//filtering items based on thier category
...
function displayButtons() {
  ...
  const buttons = categories.map((item) => {
    return `<button type="button" class="filter-btn" data-type=${item}>${item}</button>`
  }).join("");

  btnContianer.innerHTML = buttons;

  const filterBtns = document.querySelectorAll(".filter-btn");
  filterBtns.forEach(filterFunction);
}
